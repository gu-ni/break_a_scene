...
def load_image_mask_pair(instance_data_root):
    """
    Load the single image and mask pair from the given directory.
    Returns a tuple: (image_path, mask_path)
    """
    files = os.listdir(instance_data_root)
    ...
    for file in files:
        # 마스크 파일 조건: _bg 확장자
        if any(file.endswith(f"_bg{ext}") for ext in image_extensions):
            mask_file = file
        # 대상 이미지 파일 조건: _bg 없는 이미지
        elif file.lower().endswith(image_extensions):
            image_file = file

    if image_file is None or mask_file is None:
        raise ValueError(f"No valid image-mask pair found in {instance_data_root}")
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        img_name, mask_name = load_image_mask_pair(instance_data_root)
        
        # 경로 설정
        img_path = os.path.join(instance_data_root, img_name)
        mask_path = os.path.join(instance_data_root, mask_name)

        # 이미지 로드 및 변환
        self.instance_image = self.image_transforms(Image.open(img_path))

        # 마스크 로드 및 변환
        curr_mask = Image.open(mask_path)
        curr_mask = self.mask_transforms(curr_mask)[0, None, None, ...]
        self.instance_masks = torch.cat([curr_mask])

        print(f"Loaded image: {img_name} and mask: {mask_name}")
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    if self.args.with_prior_preservation:
                        ...
                        if self.args.apply_masked_loss:
                            ...
                            model_pred = model_pred * downsampled_mask.clone()
                            target = target * downsampled_mask.clone()
                        ...
                        if self.args.apply_masked_loss:
                            ...
                            # )
                            downsampled_mask = F.interpolate(
                                input=max_masks, size=(model_pred.shape[-2], model_pred.shape[-1]), 
                    ...
                    if self.args.lambda_attention != 0:
                        ...
                        for batch_idx in range(self.args.train_batch_size):
                            ...
                            if curr_cond_batch_idx >= len(batch["input_ids"]):
                                curr_cond_batch_idx = 0  # 인덱스 범위를 벗어나면 첫 번째로 고정
                            ...
                            for mask_id in range(len(GT_masks)):
                                ...
                                asset_attn_mask = agg_attn[..., asset_idx].clone()
                                asset_attn_mask = (
